...
source: "https://clawhub.1688.com/community/skill/10816"
author:
published:
created: 2026-07-16
...
Agent 根据用户意图， **先读 reference → 再按示例执行命令** （命令速查见上方「Tool 总览」）。 各命令在 AK 缺失等情况下会自行返回明确错误，Agent 按下方「错误处理」应对即可。
...
<!-- \[DISABLED\] 可视化商品墙（暂时注释） - \*\*可视化商品墙支持\*\*：\`markdown\` 在完整表格之后可能带有 \*\*「可视化商品墙」\*\* 小节（含本地 HTML 路径）。该小节与表格同属 \`markdown\` 字段的固定输出，\*\*须一并完整展示\*\*，并提示用户在浏览器中打开路径以使用交互界面；\*\*禁止省略该路径或删除本节\*\*。 -->
...
<!-- \[DISABLED\] 生成页面功能（暂时注释） - 当用户回复「生成页面」时，Agent 应引导用户在浏览器中打开 \`data.visual\_html\_path\` 对应的可视化商品墙 HTML 文件。该页面已内置商品卡片展示、勾选、页面内抽屉查看详情和下单等功能，可直接用于筛选和下单。 -->
...
| --- | --- |
| "AK 未配置" 或 "AK 未就绪" | **停止一切搜索尝试** ，优先执行 `python3 cli.py get_ak` 自动获取 AK；如自动获取失败，引导用户前往 [https://clawhub.1688.com/](https://clawhub.1688.com/) 获取后执行 `python3 cli.py configure YOUR_AK` 。 **禁止浏览器替代** |
...
- v1.0.0 (2026-03-27): 初始版本，包含三大核心搜索能力（text\_search、image\_search、link\_search）