...
## 2026-08-03：实验性入库阿里官方《AI点睛》说明书

- 通过固定电商浏览器 CDP `127.0.0.1:9223` 读取公开钉钉动态文档，截获结构化 `/api/document/data` 文档包；没有保存 Cookie、请求头或访问令牌。
- 归档约 5,600 字结构化正文、5 张表格和 16 张原始 PNG 图片，并为 131 个来源单元建立逐项覆盖矩阵。
- 经用户确认，在 `domains/电商运营/02-淘宝天猫/万相台无界白皮书/` 建立阿里官方说明书资料层，与课程型“淘宝智能推广与人群运营系统”保持独立并双向关联。
- 新建《AI点睛：关键词推广的意图匹配与操作指南》，整理搜索需求匹配、自然语言流量表达、开通与需求管理、屏蔽词、报表口径、数量/时间限制和 Agent 诊断清单；16 张信息图片全部嵌入并配图注。
- 新建 `queries/淘宝AI点睛关键词推广.md`，更新淘宝天猫、电商运营和总索引。
- 原始证据与审计材料：`raw/webpages/taobao/ai-dianjing-search-promotion-2026-08-03/`、`raw/assets/ai-dianjing-search-promotion-2026-08-03/` 与 `_meta/extraction-notes/ai-dianjing-search-promotion-2026-08-03/`。

## 2026-08-03：入库 GigsGigsCloud 东京 Shadowsocks 节点部署与运维

- 将真实购买、退款、SSH 初始化、线路基准、Shadowsocks 部署、安全组、Mihomo 接入、代理测速、客户端兼容和分享治理过程编译为可复用工具链。
- 经用户确认，在 `domains/AI Agent工程/05-工具链/05-网络与远程节点/` 新建分类入口和《GigsGigsCloud 东京 Shadowsocks 节点部署与运维》。
- 新建 `queries/VPS代理节点部署与排障.md`，为选线路、搭节点、下载慢、丢包、SSH 端口、`DynamicUser`、TCP/UDP 443 和 V-Ninja 不兼容提供统一路由。
- 归档 4 张原始控制台截图；因包含 IP、端口、服务标识或 SSH 指纹，原图只保留在私有 raw。正式页仅嵌入遮盖地址栏、实例标识和 SSH 外部端口的安全组派生图。
- 保留 2026-08-03 实测快照：约 42–45 ms 基础延迟、0% 丢包、约 46 Mbps 真实代理吞吐；套餐、退款政策、系统与线路质量均标记为执行时复核。
- 原始证据与审计材料：`raw/articles/gigsgigscloud-shadowsocks-node-2026-08-03/`、`raw/assets/gigsgigscloud-shadowsocks-node-2026-08-03/` 与 `_meta/extraction-notes/gigsgigscloud-shadowsocks-node-2026-08-03/`。

## 2026-08-03：启动万相台无界白皮书“我要推广”批量编译

- 通过 `tbcli document tree` 归档官方总纲的 436 个目录节点，并经用户确认将“我要推广”按十个业务模块逐篇编译；只在首份内容就绪时建立模块目录，不预建空目录。
- 建立 `01-关键词推广/` 模块索引，将既有《AI点睛》说明迁入模块，并补齐 16 张图片的逐图审计清单；旧路径引用已同步调整。
- 首篇新增《AI选词：大模型选词入口与使用边界》，覆盖候选词生成、选词视角、新建/存量计划入口、市场字段、AI点睛分工和 Agent 使用边界。
- AI选词来源共登记 16 个来源单元：14 个正式化，商家群标题与二维码 2 个单元仅保留 raw；4 张图片全部归档，3 张知识图片进入正式页，二维码不嵌入。
- 对“秒级生成 100+ 相关词”“节省 90% 手工操作”等数字明确标记为官方宣传口径，不作为独立实测结论或效果承诺。
- 原始资料与审计材料：`raw/webpages/taobao/ai-keyword-selection-2026-08-03/`、`raw/assets/ai-keyword-selection-2026-08-03/`、`_meta/extraction-notes/ai-keyword-selection-2026-08-03/`；总纲目录归档与批次规划位于 `raw/webpages/taobao/wanxiangtai-wujie-woyaotuiguang-2026-08-03/` 和 `_meta/extraction-notes/wanxiangtai-wujie-woyaotuiguang-2026-08-03/`。

## 2026-08-03：编译关键词百宝箱四类词包与阶段策略

- 使用 `tbcli document get` 读取阿里官方《关键词推广-关键词百宝箱｜教你如何覆盖“全部”消费者搜索意图》，归档结构化正文与 6 张原图；文档核心知识主要存在于图片中的模型、对比和阶段策略表。
- 采集时发现钉钉文档图片使用 `alidocs.oss-cn-zhangjiakou.aliyuncs.com/lark/`；为 `tbcli` 增加 HTTPS、固定主机和固定路径的严格可信规则，并补充伪造后缀域名、HTTP 和错误路径拒绝测试，完整测试 49/49 通过。
- 新建《关键词百宝箱：四类词包与分阶段投放策略》，把精确自选、关键词组合、广泛自选和流量智选整理为分层覆盖框架，并明确“覆盖全部”不是字面 100% 覆盖承诺。
- 完整保留关键词组合的商品/词包/创意/出价流程，流量智选的新品期、成长期、爆款期策略，精确与广泛匹配的“女连衣裙”示例，以及三类商家痛点决策。
- 共登记 36 个来源单元：35 个正式化，原语雀历史链接 1 个仅保留 raw；6 张知识图片全部归档、嵌入并建立可搜索语义锚点。
- 不自行补造新品/成长/爆款划分阈值，也不补造高点击、高转化、低点击成本和“优质词”的算法口径；执行前需核对当前后台。
- 原始资料与审计材料：`raw/webpages/taobao/keyword-treasure-box-2026-08-03/`、`raw/assets/keyword-treasure-box-2026-08-03/`、`_meta/extraction-notes/keyword-treasure-box-2026-08-03/`。

## 2026-08-03：融合自选词广泛与精准匹配官方说明

- 使用 `tbcli document get` 归档《关键词推广-自选词（广泛匹配 vs 精准匹配）》的 568 字结构化正文与 5 张原图。
- 现有《关键词百宝箱》已包含精确/广泛基础定义，本轮采用 `extend-existing-with-section`，没有建立重复正式页；新增后台入口、方括号识别方式、五类推荐词和三类自定义买词来源。
- 明确两份官方资料分别使用“精准匹配”和“精确匹配”，将其记录为同一自选词匹配方案的不同文案，实际执行以当前后台标签为准。
- 建立综合推荐词、精准引流词、类目优选词、行业机会词和助攻词的定义与使用边界；助攻词按平台 MTA 多触点归因理解，不升级为严格因果贡献。
- 截图中的 `0/180` 只记录为 2026-04-16 UI 快照，不固化为长期关键词上限；相关性、预估展现、点击率、点击转化率和市场平均出价均作为辅助字段，不承诺效果。
- 共登记 24 个来源单元：20 个正式化、3 个融合进既有章节、1 个历史链接 raw-only；5 张图片全部归档、嵌入并添加可检索语义锚点。
- 原始资料与审计材料：`raw/webpages/taobao/self-selected-keywords-matching-2026-08-03/`、`raw/assets/self-selected-keywords-matching-2026-08-03/`、`_meta/extraction-notes/self-selected-keywords-matching-2026-08-03/`。

## 2026-08-03：补全关键词组合卖点主题词与流量放大系数

- 使用 `tbcli document get` 归档阿里官方《关键词推广-关键词组合｜卖点主题词推荐更精准》的 804 字结构化正文、1 个表格和 7 张原图。
- 该来源是现有关键词组合章节的详细说明，本轮采用 `extend-existing-with-section`，没有建立重复正式页；补充后台入口、六个词包字段、动态换词机制、流量放大系数公式和精确买词/词包买词对比。
- 保留“保湿套装”算例：原词精确覆盖 PV80，词包总覆盖 PV200，流量放大系数为 2.5；明确该指标描述覆盖倍数，不代表点击、成交或利润同比例提升。
- 官方宣传图中的“展现、点击、成交笔数平均提升 20%+”“CTR 平均提升 5%+”被保留为来源宣称；因缺少样本、周期、基线、对照和显著性，不作为单店效果保证。
- 将投放方法升级为商品匹配→词包选择→创意承接→出价调整的四环闭环；建议出价只作为初始测试参考。
- 共登记 30 个来源单元：21 个正式化、5 个融合、3 个 raw-only、1 个因时效性省略；7 张独立图片全部归档，6 张进入正式页，已过期群二维码仅保留 raw。
- 原始资料与审计材料：`raw/webpages/taobao/keyword-combination-selling-points-2026-08-03/`、`raw/assets/keyword-combination-selling-points-2026-08-03/`、`_meta/extraction-notes/keyword-combination-selling-points-2026-08-03/`。

## 2026-08-03：编译流量智选动态买词与人工治理闭环

- 使用 `tbcli document get` 归档阿里官方《关键词推广-流量智选词包》的 1,548 字结构化正文和 8 张原图。
- 现有关键词百宝箱只承担机制总览；本来源有潜力词、托管、屏蔽和额度的独立运行闭环，因此新建《流量智选词包：目标、报告、托管与屏蔽词》，并从总览页双向链接。
- 完整保留捡漏、类目优选、好词优选三种目标，以及“系统探索→潜力词沉淀→无展现词托管→低质量词屏蔽→次日复核”的人工治理闭环。
- 保留无展现词手动/自动托管和 30/60/90 天界面快照；明确托管只是释放配额并保留命中购买机会，不保证未来流量或成交。
- 固化屏蔽词四个易错边界：只对当前单元生效、作用于系统词包、次日正式生效、中心词屏蔽不等于广泛匹配范围；并保留近 7 天价值数据和词多时可能只展示 20% 的计算边界。
- 保留中心词 10 个、精准词 50 个基础上限，以及近 30 天消耗超过 2,000/10,000 元可申请 100/200 个精准屏蔽词的历史规则；明确需客服工单申请且执行前复核。
- 共登记 45 个来源单元：44 个正式化、1 个历史链接 raw-only；8 张知识/操作图片全部归档、嵌入并建立语义锚点。
- 原始资料与审计材料：`raw/webpages/taobao/smart-traffic-keyword-package-2026-08-03/`、`raw/assets/smart-traffic-keyword-package-2026-08-03/`、`_meta/extraction-notes/smart-traffic-keyword-package-2026-08-03/`。

## 2026-08-04：串行编译关键词推广下一批 10 份官方文档

- 按“我要推广”总纲目录顺序，用 `tbcli document get` 串行取得 10 份可读取文档；“如何选品”是无结构化正文的目录壳，未用空页凑数，继续向后补足至 10 份正文。
- 批次覆盖流量智选异常流量过滤、3 份新品资料、2 份搜索机会品资料、好货快投/智能选品、如何选人迁移页、人群拿量和目标人群强化；共归档 39 张独立图片。
- 没有机械生成 10 个正式页：异常流量过滤并入既有流量智选页，其余来源按业务主题融合为《新品推广》《搜索机会品》《智能选品》《关键词推广人群》4 页。
- 新品页区分新品潜力、已开放冷启能力、测试期成交加速工具和高门槛新品发售卡位；所有倍数、提升率、预算、消耗和资格均按官方历史快照处理。
- 搜索机会品页建立转化、引流、店铺贡献和洼地流量四类机会框架，解释机会分，并区分推广配方、智能选品、自定义选品和流量金卡四类入口。
- 智能选品页以批量测款/新品扶持、成交转化、人群触达、低成本补量四类当前目标组织，并把每日汰换与历史潜力新品 7 天汰换规则分开，避免跨版本拼接。
- 人群页用“单元人群展现占比→单个人群渗透率→扩词或加价→目标人群强化”的诊断主线，明确 `80%`、3 天和平均出价公式属于来源快照。
- 共登记 39 个来源单元：36 个正式化、1 个融合、2 个群/报名/奖励单元 raw-only；39 张图片全部分析，31 张嵌入正式页，8 张重复界面图融合处置，无 unresolved。
- 原始资料分布在 `raw/webpages/taobao/*-2026-08-04/` 的本批 10 个目录，审计材料集中于 `_meta/extraction-notes/wanxiangtai-keyword-promotion-batch-2026-08-04/`。

## 2026-08-04：恢复万相台无界白皮书官方层级

- 复核 436 节点总纲后确认：既有主题页内容完整，但“官方来源目录层”没有落地，导致产品手册、通用产品能力、如何选词、如何选品和如何选人的父子关系被平铺。
- 采用“官方目录层 + 主题知识层”双层结构：前者忠实保存来源路径和编译状态，后者继续按经营问题融合内容，不搬动或重写已经验证的 8 篇主题页。
- 新建白皮书官方目录、关键词推广产品手册、通用产品能力、如何选词、如何选品、如何选人 6 个结构索引；只为已有可用内容的分支创建页面，其余分支留在目录表中标记待编译。
- 把 AI选词、关键词百宝箱、自选词、关键词组合和流量智选挂回“如何选词”；把新品、机会品和智能选品挂回“如何选品”；把人群拿量与目标人群强化挂回“如何选人”。
- 明确 AI点睛在官方树中与“如何选词”同级；它与 AI选词业务相关，但不能因此改写来源归属。
- 后续批量编译必须先定位官方父节点，再决定新建、融合或扩展主题页，禁止继续按抓取顺序扁平铺设。

## 2026-08-04：按官方层级编译关键词推广第二批 10 份文档

- 用 `tbcli document get` 串行归档智能出价人群、天气标签、趋势榜单、行业偏好、覆盖率、人群价值、智能拉新、AI 点睛升级、SKU 裂变和分时折扣 10 份阿里官方文档；登录与验证状态正常。
- 没有按抓取顺序生成 10 个平级文件：覆盖率与人群价值并入拿量诊断页，5 份选人资料融合为人群选择页，AI 点睛升级并入既有专题，SKU 裂变与分时折扣各建独立页。
- 新建《关键词推广人群选择》《SKU 裂变》《分时折扣》3 个正式页面，扩展《AI 点睛》和《关键词推广人群》2 个既有页面，并把结果挂回“通用产品能力→如何选人”等官方父节点。
- 共登记 36 个知识单元：34 个正式化、1 个融合、1 个二维码单元仅保留 raw；42 张原图全部归档，22 张知识图片嵌入正式页，20 张重复、历史界面或二维码图片只保留 raw。
- 所有数值、入口与界面均按 2026-08-04 官方快照处理；价值系数、覆盖率、点击门槛、素材上限、生成时长和优质时段推荐都不升级为跨账户效果承诺。
- 原始资料位于 `raw/webpages/taobao/*-2026-08-04/` 的本批 10 个目录，审计材料集中于 `_meta/extraction-notes/wanxiangtai-keyword-promotion-batch-2-2026-08-04/`。

## 2026-08-04：编译关键词推广第三批 10 份文档并补齐产品手册分支

- 使用 `tbcli document get` 串行归档计划预警、最低日预算、搜索意图全域追投、历史搜索人群追投、搜索卡位迁移、搜索机会品、两份相似品跟投和两份智能出价资料；“自定义选品”父节点只有 1 个字符的目录壳，不计入 10 篇有效正文。
- 新建《预算与计划预警》《搜索意图全域追投》《相似品跟投》《智能出价目标》4 个主题页，并新建“自定义推广”“搜索卡位与市场渗透”两个官方分支索引；旧版机会品资料作为版本说明并入既有页面。
- 计划预警页区分撞线与低展现，保留 `70%`、`100`、最低日预算 `50 元` 和预算跨日保留等版本规则，不把后台建议升级为利润判断。
- 全域追投页把历史搜索人群追投作为版本线合并，明确猜你喜欢/站外资源、按点击扣费、黑盒创意、新旧关键词归因和结案四模块。
- 相似品跟投页以新品跟爆、竞争拉新、流失防守、乘胜追击和相似跟投五策略为主线，同时保留旧六分类、2.0 升级和 `3 万/5 万` 权限门槛冲突；来源中的倒置点击率公式被排除。
- 智能出价页用“总成交/直接成交 × 放量/控成本”建立四格决策，并补充稳定 ROI、加购收藏、点击、市场渗透、相似品跟投和 T+1 成功率边界。
- 共登记 50 个知识单元：46 个正式化、3 个融合、1 个群/二维码单元 raw-only；76 张图片全部归档，37 张嵌入正式页，39 张历史、重复、装饰或缺少样本口径的图片仅保留 raw。
- 原始资料位于 `raw/webpages/taobao/*-2026-08-04/` 的本批目录，审计材料集中于 `_meta/extraction-notes/wanxiangtai-keyword-promotion-batch-3-2026-08-04/`。

## 2026-08-04：关键词推广第四批融合与批量提取容错升级

- 持续使用 `tbcli document get --close-tab` 从“我要推广”总纲按官方顺序提取；每篇成功或可识别的非正文节点处理后关闭来源标签页。
- 修复来源把文档链接误标为图片时整篇失败的问题：tbcli 现在跳过不可信的非图片资源，同时原子保存正文、表格、有效图片和跳过原因；测试 51/51 通过。
- 本批先正式处理 10 个有效来源，新建《关键词推广创意》《自定义推广：三类需求与操作总览》2 页，并扩展预算、流量智选、人群、AI点睛和智能出价既有页面。
- 自定义推广按“商品由谁选、出价由谁控”整理成三类组合；创意页明确创建后维护、采纳/复制同步关系、默认智能创意和素材数量边界。
- 预算页新增日均预算 7 日周期和高转化意向流量探索权益；流量智选页新增 2.0 与原搜索超车的版本迁移；智能出价页新增净成交内测目标。
- 共登记 25 个知识单元：18 个正式化、6 个融合、1 个群号/案例宣传单元 raw-only；148 张来源图片完整保留，关键流程图进入正式页，其余保留 raw。
- 所有效果提升数字、500% 溢价、100 元门槛、类目素材上限、内测和灰度资格均按官方快照处理，不升级为跨账户保证。
- 审计材料：`_meta/extraction-notes/wanxiangtai-keyword-promotion-batch-4-2026-08-04/`；修改前备份：`/Users/pechen/AI/Research/output/wiki-backups/wanxiangtai-batch4-before-formal-2026-08-04/`。

## 2026-08-04：完成万相台无界“我要推广”全量编译

- 依照官方总纲的十个一级模块，完成 431 个 `alidoc` 节点的全量盘点；401 个节点已通过 `tbcli document get --close-tab` 归档，30 个目录、申请表或无可解析正文节点已记录为有理由省略，没有从标题伪造知识。
- 在原有关键词、人群、货品全站、店铺直达、内容营销和活动运营基础上，完成线索推广的承接、CRM、数据诊断、合规和行业指南，并新建小店成交宝、活动助手和案例魔方独立模块。
- 正式层继续采用“官方目录层 + 主题知识层”：十个一级模块全部可导航，共有 62 个经营主题页，并保留产品手册、CRM、玩法和活动等官方分支。
- 401 个原始包保存 Markdown、纯文本、表格、图片清单和下载图片；图片清单共登记 2,027 张，所有原文提取完成后对应 Chrome 标签页均已关闭。
