# Source Profile｜跨境电商：赋能中小企业出海

| field | value |
| --- | --- |
| title | 跨境电商：赋能中小企业出海 |
| author | 洪勇 |
| publisher | 清华大学出版社 |
| publication_date | 2026-03-14T16:00:00+00:00 |
| ISBN | 9787302707271（版权页识别） |
| language | zh |
| source_format | EPUB |
| raw_source_path | `raw/books/cross-border-ecommerce-sme-globalization-2026/source.epub` |
| import_date | 2026-06-13 |
| sha256 | `b0ef16f0933c6bf4fa863259a7de718256fc62e583d3a79dc38758cf404f9f98` |
| adapter | `llm-wiki-ingest/adapters/book.md` |
| target_domain | `domains/电商运营/` |
| formal_path | `domains/电商运营/20-跨境电商/01-中小企业跨境出海/domains/视觉制作/05-小红书风格AI生图/index/` |
| extraction_issue | EPUB TOC 仅识别为 Start，章节依据正文标题重建；部分章节源文件标题为临时 PDF 路径；图片仅按图号和上下文保留，不逐张复刻。 |
| time_sensitivity | medium：平台、支付、物流、合规和营销工具具有当前规则复核需求。 |

## Source positioning

本书围绕跨境电商如何赋能中小企业出海，覆盖市场选择、产品本地化、品牌建设、数字化营销、物流、本地化运营、知识产权、支付、合规与风险管理。入库目标不是复刻章节摘要，而是沉淀为可供 Agent 做中小企业跨境出海诊断、方案设计和风险复核的知识路径。
