...
- 紧凑接口索引：`raw/api/wangdian-openapi/wangdian-openapi-compact-domains/视觉制作/05-小红书风格AI生图/index-2026-06-03.json`
...
- [[domains/电商运营/30-ERP与系统工具/01-旺店通开放平台API/03-旺店通订单类API使用手册|旺店通订单类 API 使用手册]]：订单创建、查询、修改、物流同步、订单标签和小时达相关接口。
- [[domains/电商运营/30-ERP与系统工具/01-旺店通开放平台API/04-旺店通库存类API使用手册|旺店通库存类 API 使用手册]]：库存查询、全量库存、库存同步、出入库、盘点、调拨、分拣、运营仓等接口。
- [[domains/电商运营/30-ERP与系统工具/01-旺店通开放平台API/05-旺店通货品类API使用手册|旺店通货品类 API 使用手册]]：平台货品、系统货品、组合装、分类、条码和唯一码打印接口。
- [[domains/电商运营/30-ERP与系统工具/01-旺店通开放平台API/06-旺店通基础类API使用手册|旺店通基础类 API 使用手册]]：供应商、标签、物流、员工、店铺、仓库和业务单据操作接口。
- [[domains/电商运营/30-ERP与系统工具/01-旺店通开放平台API/07-旺店通售后类API使用手册|旺店通售后类 API 使用手册]]：退货入库、原始退款单、退换管理、系统售后单和退货入库确认接口。
- [[domains/电商运营/30-ERP与系统工具/01-旺店通开放平台API/08-旺店通采购类API使用手册|旺店通采购类 API 使用手册]]：采购单、采购退货单、采购申请单的查询和创建接口。
