---
title: 达摩盘 AI 内部工具与竞品数据挖掘手册
type: playbook
created: 2026-06-20
updated: 2026-06-22
domain: 电商运营
tags: [ecommerce, taobao, tmall, dmp-ai, competitive-intelligence, data-tooling]
sources:
...
# 达摩盘 AI 内部工具与竞品数据挖掘手册

## 使用定位

这页把本地研究文档 `DMP_AI_internal_tools.md` 编译为 Wiki 正式手册。它用于未来在达摩盘/DMP AI 对话中继续挖掘内部工具、验证工具参数、判断竞品销售数据、搜索词数据、推广数据和人群数据分别应该走哪条工具路线。

未来 Agent 在处理“DMP AI 竞品数据挖掘”类任务时，应先读 [[queries/达摩盘AI竞品数据挖掘|达摩盘 AI 竞品数据挖掘查询入口]]，再回到本手册查完整字段、调用话术、返回样例和边界测试。

## 关键结论

- `tool_item_benchmark_dataquery` 是当前最强的竞品商品销售/推广基准路线，`type=ad`、`type=cust_type`、`type=free` 曾返回精确同行商品行，且可通过 `data_id` 继续 SQL 查询。
- `tool_crowd_audience_insight` 是当前最强的竞品人群路线，支持竞品商品浏览、加购、收藏、购买，以及竞品店铺购买、浏览未购买等自然语言人群条件，测试中返回精确人数和画像比例。
- `tool_competitor_explain_mining` 和 `tool_competitive_detect_mining` 更适合找竞品和解释竞品关系，其中关键词路线可拿到明文关键词列表，但不是直接的搜索词效果报表。
- `tool_e_commerce_crowd_advertising_dataquery` 对自家广告诊断有用，但广告主 ID 是硬边界，竞品商品在本广告主上下文返回空，不应作为竞品推广抽取主路线。
- `tool_common_business_asset_dataquery`、`tool_data_asset_guide_retrieval`、`tool_sql_query` 和各类 VIEW/DEEPLINK 数据资产是继续扩展的关键，需要用元数据检索、业务资产查询和 SQL 联动。

## 时效与安全边界

- 来源观察日期为 2026-06-20，属于已登录达摩盘/DMP AI 会话中的工具行为记录；平台工具、字段、权限和脱敏策略可能变化。
- 原文中的自有广告主 ID 已以 `1395****73` 形式保留遮蔽；未来新增记录也应继续遮蔽自有会员/广告主 ID。
- 手册中的 `{BASE_ITEM_ID}`、`{COMP_ITEM_ID}`、`{DATA_ID}` 等 `{...}` 表达都是模板占位符；实际在 DMP AI 对话中使用时，应替换成当次真实值并删除花括号。正式 wiki 不记录真实本品商品 ID 或完整会员 ID。
- 这页保留的是工具研究和操作知识，不代表 Codex 可直接调用阿里内部工具；实际调用仍需在 DMP AI 对话中用工具名和参数话术触发。

## 完整原文编译

以下保留源文档的完整 Markdown 结构、表格、代码块、调用参数、返回样例、失败/边界测试和后续线索。为避免任何信息遗漏，本节不做压缩改写。
