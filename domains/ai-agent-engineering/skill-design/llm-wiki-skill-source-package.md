...
- [[domains/ai-agent-engineering/skill-design/codex-skill-inventory|llm-wiki]]：总入口，负责 orientation、query-pack、轻量查询和健康检查。
- [[domains/ai-agent-engineering/skill-design/codex-skill-inventory|llm-wiki-ingest]]：唯一入库入口，承接网页、剪藏、API、书籍、课程转写、PPT、XMind、表格、Markdown 等来源。
- [[domains/ai-agent-engineering/skill-design/codex-skill-inventory|llm-wiki-audit-and-optimization]]：编译、路由、推理质量审阅，包含 Obsidian Route Audit。
- [[domains/ai-agent-engineering/skill-design/codex-skill-inventory|llm-wiki-recompile-runner]]：修复 shell/thin pages 和重编译 learning path。
- [[domains/ai-agent-engineering/skill-design/codex-skill-inventory|ai-agent-skill-registry-sync]]：同步跨 Agent skill 注册页。
...
- 使用时：构造 query-pack，按 `queries/`、Agent 使用模板、index、细页的顺序路由。
