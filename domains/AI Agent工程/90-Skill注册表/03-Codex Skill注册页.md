...
- `llm-wiki-recompile-runner` (个人/项目自定义 / local)：Orchestrate repair and memory-first reorganization of existing LLM Wiki domains or learning/source packages that contain…
...
- `editable-poster-psd-rebuild` (个人/项目自定义 / local)：Turn an AI-generated ecommerce poster image with text into a Photoshop PSD with a no-text background, hidden original reference layer, and editable Photoshop text layers using approved fonts.
...
- 功能检索描述：Turn an AI-generated ecommerce poster image with text into a Photoshop PSD with a no-text background, hidden original reference layer, and editable Photoshop text layers using only user-approved or free-commercial whitelist fonts. Use when the user asks to remove AI-generated poster text, rebuild typography in PS/PSD, make text editable, preserve layout from a reference image, or avoid commercial font infringement.
- 输入 / 触发方式：带文案 AI 海报图片路径、去文案底图需求、PSD 可编辑文字层需求、字体合规/授权字体要求、Photoshop 版式重建需求。
- 检索关键词：editable-poster-psd-rebuild 可编辑海报 PSD 重建 AI海报去字 字体合规 Photoshop type layer 思源宋体 思源黑体 GPT Image 2 电商海报 可编辑文字层 `/Users/pechen/.codex/skills/editable-poster-psd-rebuild/SKILL.md` local
