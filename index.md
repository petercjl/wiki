...
- [[domains/量化交易/01-聚宽/04-聚宽社区策略取证与日志瘦身方法|聚宽社区策略取证与日志瘦身方法]]：把社区策略回测入库拆成收益/交易/持仓导出与 summary-only 审计日志的取证流程。
...
- [[domains/量化交易/03-策略案例/index|量化策略案例库]]：收录已经完成代码、回测导出、审计日志和本地分析的策略案例。
- [[domains/量化交易/03-策略案例/01-QMT四季发财ETF轮动策略案例|QMT四季发财 ETF 轮动策略案例]]：ETF 单标的轮动策略案例，记录动量稳定性得分、过滤/止损规则、2020-2025 回测证据、收益来源和实盘边界。
